...
export default async function CafeTakeawayWaiterPage({
  params,
}: {
  ...
  const [t, session, settings, cart, waiters, canDashboard] = await Promise.all([
    getDictionary(),
    auth(),
    getSystemSettings(),
    ...
    <div className="flex min-h-dvh flex-col">
      ...
      <div className="space-y-6 p-4">
        <WaiterPickerGrid
          ...
          workspaceHref={`/caisse/cafe/takeaway/${orderId}`}
        />
      </div>
    </div>
  );
