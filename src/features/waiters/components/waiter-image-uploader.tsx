...
export type WaiterPhoto = { publicId: string; secureUrl: string };
...
export function WaiterImageUploader({
  value,
  onChange,
  name,
  disabled,
}: {
  value: WaiterPhoto | null;
  onChange: (photo: WaiterPhoto | null) => void;
  name: string;
  disabled?: boolean;
}) {
  const { t } = useLocale();
  const tp = t.waiters.photo;
  ...
  async function handleFile(file: File | undefined) {
    ...
    setIsUploading(true);
    try {
      const signRes = await fetch("/api/cloudinary/sign-waiter-photo", {
        method: "POST",
      });
